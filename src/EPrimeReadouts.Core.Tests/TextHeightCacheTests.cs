...
public class TextHeightCacheTests
{
    ...
    public async Task HeightIsMeasuredOnlyWhenTextWidthOrRevisionChanges()
    {
        ...
        float first = cache.Get("Pool help", 200f, 1, state, Measure);
        state.Height = 99f;
        float reused = cache.Get("Pool help", 200f, 1, state, Measure);
        float resized = cache.Get("Pool help", 240f, 1, state, Measure);
        float rescaled = cache.Get("Pool help", 240f, 2, state, Measure);
